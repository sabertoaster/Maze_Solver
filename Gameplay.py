...
class Gameplay:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if self.solution_flag:
                        ceil_rect = pygame.Rect(self.player.grid_pos[0] * self.cell_size,
                                                self.player.grid_pos[1] * self.cell_size, self.cell_size,
    ...
    def init_background(self):
        ...
        for i in range(self.maze_row):
            for j in range(self.maze_col):
                ...
                if self.maze_toString[i][j] == '#':
                    wall = morph_image(RESOURCE_PATH + tuan[np.random.randint(0, len(tuan))], (self.bg_cell_size, self.bg_cell_size * 2)).convert_alpha()
                    maze_surface.blit(wall, pos)
                elif self.maze_toString[i][j] == 'S':
    ...
    def toggle_panel(self):
        ...
        def at_refresh():
            ...
            copy_screen.blit(self.minimap.new_background, self.minimap.display_pos,
                             (self.minimap.cut_start_pos, self.minimap.cut_area))
