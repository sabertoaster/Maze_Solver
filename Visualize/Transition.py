...
class Transition:
    ...
    def circle(self, pos, zoom_in=True): # pos: x, y
        ...
        max_radius = max(calc_distance(pos, (0,self.resolution[0])),
        ...
        if zoom_in:
            ...
            for _ in range(rate):
                max_radius -= 1*1*radius / rate
                filter = create_circular_mask(h=self.resolution[0], w=self.resolution[1], center=pos, radius=max_radius)
